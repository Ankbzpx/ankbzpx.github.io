...
Ever since the badge generation project, I have been fascinated by the process of 3D mesh modeling--the artistry of duplicating and offsetting mesh edge loops. It allows concise and meticulous control, which are hard to replicate for other geometry representation, i.e. the implicit one preferred by state of the art 3D generative methods. However, such editing doesn't come cheap--it requires quad dominant mesh with feature aligned edge loops, the property that mesh extracted from volume data does not possess.
...
Thus, its immediate application is to save the cross field generation step of quadrilateral remeshing.
...
#### Bonus
This my attempt to solve continuous seamless volume parameterization. It's... crappy to say the least, but a pretty fun experience!
