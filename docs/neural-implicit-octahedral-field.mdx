...
Ever since the badge generation project, I have been fascinated by the process of 3D mesh modeling--the artistry of duplicating and offsetting mesh edge loops. It allows concise and meticulous control, which is hard to replicate with other geometry representation, i.e. the implicit one preferred by state of the art 3D generative methods. However, such editing doesn't come cheap--it requires quad dominant mesh with feature aligned edge loops, the property that mesh extracted from volume data does not possess.
...
$$
where $f: V_{param} \to V_{\mathbb{R}^3}$

<InverseParamViewer name='cube_twist' />

<InverseParamViewer name='fandisk_rot' />
